...
test_defaults_and_banks (void)
{
  ...
  /* S&P F7 asks for the number — it used to double-key the callsign. */
  k = logfl_macro_set_key (&set, LOGFL_MACRO_BANK_SNP, 6);
  g_assert_cmpstr (k->caption, ==, "NR");
  g_assert_cmpstr (k->tmpl, ==, "NR?");
